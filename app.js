...
const OFFLINE_STATE_KEY = "sorellaTeaPOS_offline_state_v3";
const OUTBOX_KEY = "sorellaTeaPOS_outbox_v3";
let offlineMode = false;
function setSyncStatus(isOffline) {
  offlineMode = !!isOffline;
  const el = document.getElementById("syncStatus");
  if (!el) return;
  el.textContent = isOffline ? "● Offline — saved on iPad" : "● Online";
  el.classList.toggle("offline", !!isOffline);
}
function snapshotState() {
  return { categories:state.categories, series:state.series, products:state.products, ingredients:state.ingredients, recipes:state.recipes, expenses:state.expenses, heldOrders:state.heldOrders, completedOrders:state.completedOrders, movements:state.movements, settings:state.settings||[] };
}
function saveOfflineSnapshot() { try { localStorage.setItem(OFFLINE_STATE_KEY, JSON.stringify(snapshotState())); } catch(e) {} }
function readOfflineSnapshot() {
  try { const raw=localStorage.getItem(OFFLINE_STATE_KEY); if(raw) return JSON.parse(raw); } catch(e) {}
  return window.SORELLA_OFFLINE_SEED ? JSON.parse(JSON.stringify(window.SORELLA_OFFLINE_SEED)) : {categories:[],series:[],products:[],ingredients:[],recipes:[],expenses:[],heldOrders:[],completedOrders:[],movements:[]};
}
function queueMutation(path, method, body) { try { const q=JSON.parse(localStorage.getItem(OUTBOX_KEY)||"[]"); q.push({path,method,body,queuedAt:new Date().toISOString()}); localStorage.setItem(OUTBOX_KEY,JSON.stringify(q)); } catch(e) {} }
function localUpsert(arr,row) { const i=arr.findIndex(x=>String(x.id)===String(row.id)); if(i>=0) arr[i]={...arr[i],...row}; else arr.push(row); return row; }
function offlineMutation(path,method,body) {
  const parts=path.split("/").map(decodeURIComponent), resource=parts[0], rid=parts[1]||null;
  if(resource==="orders" && method==="POST") {
    const items=Array.isArray(body?.items)?body.items:[]; const subtotal=items.reduce((a,x)=>a+Number(x.qty)*Number(x.price),0); const discountPercent=Math.max(0,Math.min(100,Number(body?.discountPercent)||0)); const discountAmount=Math.round(subtotal*discountPercent)/100; const total=subtotal-discountAmount; const cash=Number(body?.cash)||0;
    if(!items.length) throw new Error("Order has no items."); if(cash>0&&cash<total) throw new Error("Cash received is less than the total.");
    const now=new Date(), order={id:"OFF-"+now.toISOString().replace(/\D/g,"").slice(0,14)+Math.random().toString(36).slice(2,5),date:now.toISOString().slice(0,10),time:now.toLocaleTimeString("en-PH",{hour:"2-digit",minute:"2-digit"}),items,subtotal:String(subtotal),discountPercent:String(discountPercent),discountAmount:String(discountAmount),total:String(total),cash:String(cash),change:String(Math.max(0,cash-total)),payment:body?.payment||"Cash",cashier:"",ingredientCost:"0",profit:String(total)};
    for(const item of items){ const p=state.products.find(x=>x.name===item.name); const matches=state.recipes.filter(r=>r.active!==false&&r.productName===item.name&&(r.sizeLabel===item.size||r.sizeLabel==="ALL")); for(const r of matches){const ing=state.ingredients.find(i=>i.id===r.ingredientId)||state.ingredients.find(i=>i.name===r.ingredientName);if(ing){const qty=Number(r.qty)*Number(item.qty);ing.currentStock=String(Number(ing.currentStock)-qty);state.movements.push({id:"OFFMOV-"+Date.now()+Math.random(),ingredientId:ing.id,ingredientName:ing.name,type:"Sale",qty:String(-qty),note:`Order ${order.id}`,orderId:order.id,createdAt:now.toISOString()});}}}
    queueMutation(path,method,body); saveOfflineSnapshot(); return order;
  }
  if(resource==="held-orders"&&method==="POST"){const row={id:"HOLD_"+Date.now().toString(36),items:body.items||[],discountPercent:String(body.discountPercent||0),createdAt:new Date().toISOString()};state.heldOrders.push(row);queueMutation(path,method,body);saveOfflineSnapshot();return row;}
  if(resource==="held-orders"&&method==="DELETE"&&rid){state.heldOrders=state.heldOrders.filter(x=>String(x.id)!==String(rid));queueMutation(path,method,body);saveOfflineSnapshot();return {ok:true};}
  if(resource==="orders-clear"&&method==="POST"){state.completedOrders=[];queueMutation(path,method,body);saveOfflineSnapshot();return {ok:true};}
  const maps={categories:"categories",series:"series",products:"products",ingredients:"ingredients",recipes:"recipes",expenses:"expenses"};
  if(maps[resource]&&method==="POST"){const row={...(body||{}),id:rid||body?.id||id(resource.slice(0,4).toUpperCase()+"_")};queueMutation(path,method,body);saveOfflineSnapshot();return row;}
  if(resource==="ingredient-stock-in"&&method==="POST"){const ing=state.ingredients.find(x=>x.id===body.ingredientId);if(!ing)throw new Error("Ingredient not found.");const qty=Number(body.qty)||0,signed=body.type==="Waste"?-Math.abs(qty):qty;ing.currentStock=String(Number(ing.currentStock)+signed);const mv={id:"OFFMOV-"+Date.now(),ingredientId:ing.id,ingredientName:ing.name,type:body.type||"Stock In",qty:String(signed),note:body.note||"",createdAt:new Date().toISOString()};state.movements.push(mv);queueMutation(path,method,body);saveOfflineSnapshot();return mv;}
  if(resource==="ingredient-stock-set"&&method==="POST"){const ing=state.ingredients.find(x=>x.id===body.ingredientId);if(!ing)throw new Error("Ingredient not found.");ing.name=body.name??ing.name;ing.category=body.category??ing.category;ing.unit=body.unit??ing.unit;ing.currentStock=String(body.currentStock);ing.minStock=String(body.minStock??ing.minStock);ing.costPerUnit=String(body.costPerUnit??ing.costPerUnit);ing.supplier=body.supplier??ing.supplier;ing.active=body.active===undefined?ing.active:!!body.active;queueMutation(path,method,body);saveOfflineSnapshot();return ing;}
  throw new Error("This action is not available offline yet.");
}
async function flushOfflineQueue(){
  let q=[]; try{q=JSON.parse(localStorage.getItem(OUTBOX_KEY)||"[]")}catch(e){} if(!q.length)return;
  try{for(const item of q){const res=await fetch("/api/"+item.path,{method:item.method,headers:item.body?{"content-type":"application/json"}:undefined,body:item.body?JSON.stringify(item.body):undefined});if(!res.ok)throw new Error("Sync failed");}localStorage.removeItem(OUTBOX_KEY);const s=await fetch("/api/state");if(s.ok){const data=await s.json();Object.assign(state,data);const o=await fetch("/api/orders");if(o.ok)state.completedOrders=await o.json();saveOfflineSnapshot();renderAll();}setSyncStatus(false);toast("Offline changes synced.");}catch(e){setSyncStatus(true);}
}
function localBackup() {
  const snapshot = snapshotState();
  snapshot.completedOrders = state.completedOrders || [];
  snapshot.heldOrders = state.heldOrders || [];
  snapshot.movements = state.movements || [];
  return JSON.parse(JSON.stringify(snapshot));
}
function localRestore(data) {
  const keys=["categories","series","products","ingredients","recipes","expenses","heldOrders","completedOrders","movements","settings"];
  for(const key of keys) if(Array.isArray(data?.[key])) state[key]=JSON.parse(JSON.stringify(data[key]));
  saveOfflineSnapshot();
  return {ok:true};
}
async function api(path, method = "GET", body) {
  // v2.7.1 standalone build: business data is stored locally on the iPad.
  if (STANDALONE_MODE) {
    setSyncStatus(true);
    if (method === "GET") {
      const local = readOfflineSnapshot();
      if (path === "state") return local;
      if (path === "orders") return local.completedOrders || [];
      if (path === "held-orders") return local.heldOrders || [];
      if (path === "movements") return local.movements || [];
      if (path === "backup") return localBackup();
    }
    if (path === "restore" && method === "POST") return localRestore(body);
    const result = offlineMutation(path, method, body);
    setSyncStatus(true);
    return result;
  }
  try {
    ...
    setSyncStatus(false); const data = res.status === 204 ? null : await res.json(); if(path==="state"||path==="orders"||path==="held-orders"||path==="movements") setTimeout(saveOfflineSnapshot,0); return data;
  } catch(e) {
    if(navigator.onLine===false || /Failed to fetch|NetworkError|Load failed|fetch/i.test(String(e.message))){ setSyncStatus(true); if(method==="GET"){const local=readOfflineSnapshot(); if(path==="state") return local; if(path==="orders") return local.completedOrders||[]; if(path==="held-orders") return local.heldOrders||[]; if(path==="movements") return local.movements||[];} return offlineMutation(path,method,body); }
    throw e;
  }
}
...
async function init() {
  ...
  try {
    ...
    toast("Loaded local Sorella Tea data.");
  }
  renderAll();
}
...
async function completeOrder(btn) {
  ...
  try {
    const order = await api("orders", "POST", {
      items: state.order, discountPercent: state.discount, cash: state.cash, payment: state.cash > 0 ? "Cash" : "GCash",
    });
...
let manageSort = { menu: "name-asc", categories: "name-asc", series: "name-asc", ingredients: "name-asc", inventory: "name-asc", recipes: "product-asc", expenses: "date-desc", movements: "date-desc" };
function sortRows(rows, key) {
  const dir = key.endsWith("-desc") ? -1 : 1;
  const field = key.replace(/-(asc|desc)$/, "");
  return rows.slice().sort((a,b)=>{
    let av, bv;
    if(field === "name") { av=String(a.name||"").toLowerCase(); bv=String(b.name||"").toLowerCase(); }
    else if(field === "category") { av=String(a.category||"").toLowerCase(); bv=String(b.category||"").toLowerCase(); }
    else if(field === "price") { av=Number(Object.values(a.sizes||{})[0]||0); bv=Number(Object.values(b.sizes||{})[0]||0); }
    else if(field === "stock") { av=Number(a.currentStock||0); bv=Number(b.currentStock||0); }
    else if(field === "amount") { av=Number(a.amount||0); bv=Number(b.amount||0); }
    else if(field === "status") { av=a.active===false?1:0; bv=b.active===false?1:0; }
    else if(field === "product") { av=String(a.productName||"").toLowerCase(); bv=String(b.productName||"").toLowerCase(); }
    else if(field === "date") { av=new Date(a.date||a.createdAt||0).getTime(); bv=new Date(b.date||b.createdAt||0).getTime(); }
    else { av=String(a[field]||"").toLowerCase(); bv=String(b[field]||"").toLowerCase(); }
    if(typeof av === "number" && typeof bv === "number") return (av-bv)*dir;
    return String(av).localeCompare(String(bv), undefined, {numeric:true})*dir;
  });
}
function manageSortControl(current, id="manageSort") {
  return `<select id="${id}" class="manage-sort" aria-label="Sort items"><option value="name-asc" ${current==="name-asc"?"selected":""}>Name (A–Z)</option><option value="name-desc" ${current==="name-desc"?"selected":""}>Name (Z–A)</option><option value="price-asc" ${current==="price-asc"?"selected":""}>Price (low–high)</option><option value="price-desc" ${current==="price-desc"?"selected":""}>Price (high–low)</option><option value="category-asc" ${current==="category-asc"?"selected":""}>Category</option><option value="status-asc" ${current==="status-asc"?"selected":""}>Status</option></select>`;
}
...
function renderMenuManager(c) {
  const rows = sortRows(state.products, manageSort.menu);
  c.innerHTML = `<div class="manage-toolbar"><div class="form-note">Add, edit, activate or deactivate menu items. Product photos are intentionally removed for a faster, cleaner iPad checkout.</div><div class="manage-sort-wrap"><label>Sort</label>${manageSortControl(manageSort.menu)}</div></div><div class="table-wrap" style="margin-top:10px"><table class="data-table"><thead><tr><th>Item</th><th>Category</th><th>Series</th><th>Sizes / Prices</th><th>Active</th><th></th></tr></thead><tbody>
  ${rows.map(p => `<tr><td><b>${esc(p.name)}</b></td><td>${esc(canonicalCategory(p))}</td><td>${esc(p.seriesName || "—")}</td><td>${Object.entries(p.sizes || {}).map(([s, v]) => `${esc(s)} ${money(v)}`).join(" · ")}</td><td class="${p.active !== false ? "ok" : "warn"}">${p.active !== false ? "YES" : "NO"}</td><td><button class="soft editp" data-p="${p.id}" style="padding:7px 9px;border-radius:9px">Edit</button></td></tr>`).join("")}</tbody></table></div>`;
  c.querySelector('#manageSort').onchange = e => { manageSort.menu=e.target.value; renderMenuManager(c); };
  c.querySelectorAll('.editp').forEach(b => b.onclick = () => openProductEditor(b.dataset.p));
}

function renderCategories(c) {
  const sorted = sortRows(state.categories, manageSort.categories);
  const rows = manageSort.categories === "position-asc" ? state.categories.slice().sort((a,b)=>a.position-b.position) : sorted;
  c.innerHTML = `<div class="manage-toolbar"><div class="form-note">Categories determine which top navigation tab a product appears under. Disable a category to hide it from the order screen without deleting its products.</div><div class="manage-sort-wrap"><label>Sort</label><select id="manageSort" class="manage-sort"><option value="name-asc" ${manageSort.categories==='name-asc'?'selected':''}>Name (A–Z)</option><option value="name-desc" ${manageSort.categories==='name-desc'?'selected':''}>Name (Z–A)</option><option value="status-asc" ${manageSort.categories==='status-asc'?'selected':''}>Active first</option><option value="position-asc" ${manageSort.categories==='position-asc'?'selected':''}>Display order</option></select></div></div>
  ...
  c.querySelector('#manageSort').onchange=e=>{manageSort.categories=e.target.value;renderCategories(c)};
...
function renderSeriesManager(c) {
  const rows = manageSort.series === "position-asc" ? state.series.slice().sort((a,b)=>a.position-b.position) : sortRows(state.series, manageSort.series);
  c.innerHTML = `<div class="manage-toolbar"><div class="form-note">Series are optional flavor collections (e.g. Matcha Series) shown as a secondary filter row above the menu.</div><div class="manage-sort-wrap"><label>Sort</label><select id="manageSort" class="manage-sort"><option value="name-asc" ${manageSort.series==='name-asc'?'selected':''}>Name (A–Z)</option><option value="name-desc" ${manageSort.series==='name-desc'?'selected':''}>Name (Z–A)</option><option value="status-asc" ${manageSort.series==='status-asc'?'selected':''}>Active first</option><option value="position-asc" ${manageSort.series==='position-asc'?'selected':''}>Display order</option></select></div></div>
  <div class="table-wrap" style="margin-top:10px"><table class="data-table"><thead><tr><th>Order</th><th>Series</th><th>Active</th><th></th></tr></thead><tbody>${rows.map((s, i) => `<tr>
    ...
    <td><b>${esc(s.name)}</b></td><td class="${s.active !== false ? "ok" : "warn"}">${s.active !== false ? "ACTIVE" : "DISABLED"}</td>
  ...
  c.querySelector('#manageSort').onchange=e=>{manageSort.series=e.target.value;renderSeriesManager(c)};
...
function renderIngredients(c) {
  const rows=sortRows(state.ingredients,manageSort.ingredients);
  c.innerHTML = `<div class="manage-toolbar"><div class="form-note">Edit ingredients, stock thresholds, suppliers and costs.</div><div class="manage-sort-wrap"><label>Sort</label>${manageSortControl(manageSort.ingredients)}</div></div><div class="table-wrap" style="margin-top:10px"><table class="data-table"><thead><tr><th>ID</th><th>Ingredient</th><th>Category</th><th>Unit</th><th>Current Stock</th><th>Min Stock</th><th>Cost/Unit</th><th>Supplier</th><th>Active</th><th></th></tr></thead><tbody>${rows.map(i => `<tr><td>${esc(i.id)}</td><td><b>${esc(i.name)}</b></td><td>${esc(i.category)}</td><td>${esc(i.unit)}</td><td>${Number(i.currentStock).toFixed(2)}</td><td>${Number(i.minStock).toFixed(2)}</td><td>${money(i.costPerUnit)}</td><td>${esc(i.supplier || "—")}</td><td>${i.active !== false ? "YES" : "NO"}</td><td><button class="soft editIng" data-i="${esc(i.id)}" style="padding:7px 9px;border-radius:9px">Edit</button></td></tr>`).join("") || `<tr><td colspan="10">No ingredients yet.</td></tr>`}</tbody></table></div>`;
  c.querySelector('#manageSort').onchange=e=>{manageSort.ingredients=e.target.value;renderIngredients(c)};
  c.querySelectorAll('.editIng').forEach(b=>b.onclick=()=>openIngredientEditor(b.dataset.i));
}

function renderInventory(c) {
  const rows=sortRows(state.ingredients,manageSort.inventory);
  c.innerHTML = `<div class="manage-toolbar"><div class="form-note">Quickly check stock, reorder alerts and stock value.</div><div class="manage-sort-wrap"><label>Sort</label><select id="manageSort" class="manage-sort"><option value="name-asc" ${manageSort.inventory==='name-asc'?'selected':''}>Name (A–Z)</option><option value="stock-asc" ${manageSort.inventory==='stock-asc'?'selected':''}>Stock (low–high)</option><option value="stock-desc" ${manageSort.inventory==='stock-desc'?'selected':''}>Stock (high–low)</option><option value="status-asc" ${manageSort.inventory==='status-asc'?'selected':''}>Reorder alerts first</option></select></div></div><div class="table-wrap" style="margin-top:10px"><table class="data-table"><thead><tr><th>Ingredient</th><th>Unit</th><th>Current Stock</th><th>Min Stock</th><th>Cost/Unit</th><th>Stock Value</th><th>Status</th><th></th></tr></thead><tbody>${rows.map(i=>{const cur=Number(i.currentStock),min=Number(i.minStock),cpu=Number(i.costPerUnit);return `<tr><td><b>${esc(i.name)}</b></td><td>${esc(i.unit)}</td><td><b>${cur.toFixed(2)}</b></td><td>${min.toFixed(2)}</td><td>${money(cpu)}</td><td>${money(cur*cpu)}</td><td class="${cur<=min?'warn':'ok'}">${cur<=min?'REORDER':'OK'}</td><td><button class="soft editStock" data-i="${esc(i.id)}" style="padding:7px 9px;border-radius:9px">Edit Stock</button></td></tr>`}).join('')||`<tr><td colspan="8">No ingredients yet.</td></tr>`}</tbody></table></div>`;
  c.querySelector('#manageSort').onchange=e=>{manageSort.inventory=e.target.value;renderInventory(c)};
  c.querySelectorAll('.editStock').forEach(b=>b.onclick=()=>openIngredientEditor(b.dataset.i,true));
}

function renderMovements(c) {
  c.innerHTML = `<div class="manage-toolbar"><div class="form-note">Every inventory change is recorded here — completed sales deduct automatically; stock-in, waste, adjustments, returns and corrections are recorded via “+ Stock Movement”.</div><div class="manage-sort-wrap"><label>Sort</label><select id="manageSort" class="manage-sort"><option value="date-desc" ${manageSort.movements==='date-desc'?'selected':''}>Newest first</option><option value="date-asc" ${manageSort.movements==='date-asc'?'selected':''}>Oldest first</option><option value="name-asc" ${manageSort.movements==='name-asc'?'selected':''}>Ingredient (A–Z)</option></select></div></div><div class="table-wrap" style="margin-top:10px"><table class="data-table"><thead><tr><th>Date</th><th>Ingredient</th><th>Type</th><th>Qty</th><th>Note</th></tr></thead><tbody id="movementsBody"><tr><td colspan="5">Loading…</td></tr></tbody></table></div>`;
  c.querySelector('#manageSort').onchange=e=>{manageSort.movements=e.target.value;renderMovements(c)};
  api("movements").then(rows => {
    state.movements = rows;
    let ordered=rows.slice();
    if(manageSort.movements==='date-asc') ordered.sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt));
    else if(manageSort.movements==='name-asc') ordered.sort((a,b)=>String(a.ingredientName||'').localeCompare(String(b.ingredientName||''),undefined,{numeric:true}));
    else ordered.sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
    document.getElementById("movementsBody").innerHTML = ordered.map(m => `<tr><td>${new Date(m.createdAt).toLocaleString()}</td><td>${esc(m.ingredientName)}</td><td>${esc(m.type)}</td><td>${Number(m.qty) > 0 ? "+" : ""}${Number(m.qty)}</td><td>${esc(m.note || "")}</td></tr>`).join("") || `<tr><td colspan="5">No movements recorded yet.</td></tr>`;
...
function renderRecipes(c) {
  const rows=sortRows(state.recipes,manageSort.recipes);
  c.innerHTML = `<div class="manage-toolbar"><div class="form-note">Recipe links used for inventory deduction.</div><div class="manage-sort-wrap"><label>Sort</label><select id="manageSort" class="manage-sort"><option value="product-asc" ${manageSort.recipes==='product-asc'?'selected':''}>Product (A–Z)</option><option value="product-desc" ${manageSort.recipes==='product-desc'?'selected':''}>Product (Z–A)</option><option value="name-asc" ${manageSort.recipes==='name-asc'?'selected':''}>Ingredient (A–Z)</option><option value="status-asc" ${manageSort.recipes==='status-asc'?'selected':''}>Active first</option></select></div></div><div class="table-wrap" style="margin-top:10px"><table class="data-table"><thead><tr><th>Product</th><th>Size</th><th>Ingredient</th><th>Qty / Drink</th><th>Unit</th><th>Active</th></tr></thead><tbody>${rows.map(r => `<tr><td>${esc(r.productName)}</td><td>${esc(r.sizeLabel)}</td><td>${esc(r.ingredientName)}</td><td>${r.qty}</td><td>${esc(r.unit)}</td><td>${r.active !== false ? "YES" : "NO"}</td></tr>`).join("") || `<tr><td colspan="6">No recipes yet.</td></tr>`}</tbody></table></div>`;
  c.querySelector('#manageSort').onchange=e=>{manageSort.recipes=e.target.value;renderRecipes(c)};
}
function renderExpenses(c) {
  const total = state.expenses.reduce((a, e) => a + Number(e.amount || 0), 0);
  const rows=sortRows(state.expenses,manageSort.expenses);
  c.innerHTML = `<div class="kpi"><small>Miscellaneous / operating expenses</small><b>${money(total)}</b></div><div class="manage-toolbar" style="margin-top:12px"><div class="form-note">Review expenses by date, category or amount.</div><div class="manage-sort-wrap"><label>Sort</label><select id="manageSort" class="manage-sort"><option value="date-desc" ${manageSort.expenses==='date-desc'?'selected':''}>Newest first</option><option value="date-asc" ${manageSort.expenses==='date-asc'?'selected':''}>Oldest first</option><option value="amount-desc" ${manageSort.expenses==='amount-desc'?'selected':''}>Amount (high–low)</option><option value="amount-asc" ${manageSort.expenses==='amount-asc'?'selected':''}>Amount (low–high)</option><option value="category-asc" ${manageSort.expenses==='category-asc'?'selected':''}>Category</option></select></div></div><div class="table-wrap" style="margin-top:10px"><table class="data-table"><thead><tr><th>Date</th><th>Category</th><th>Description</th><th>Amount</th><th>Payment</th></tr></thead><tbody>${rows.map(e => `<tr><td>${esc(e.date)}</td><td>${esc(e.category)}</td><td>${esc(e.description)}</td><td>${money(e.amount)}</td><td>${esc(e.payment || "")}</td></tr>`).join("") || `<tr><td colspan="5">No expenses yet.</td></tr>`}</tbody></table></div>`;
  c.querySelector('#manageSort').onchange=e=>{manageSort.expenses=e.target.value;renderExpenses(c)};
}
...
function openProductEditor(pid) {
  ...
  document.getElementById("pe_save").onclick = async () => {
    ...
    const payload = {
      ...
      image: "", active: document.getElementById("p_active").value === "true",
    };
...
function copilotRespond(text) {
  ...
  if(/inactive menu|inactive items|disabled menu/.test(l)){const a=state.products.filter(p=>p.active===false);copilotMessages.push({role:"assistant",text:a.length?a.map(p=>`• <b>${esc(p.name)}</b> — ${esc(canonicalCategory(p))}`).join("<br>"):"No inactive menu items found."});return renderCopilot();}
  if(/menu.*(sort|order)|sort.*menu/.test(l)){const k=/price.*(high|desc)/.test(l)?"price-desc":/price/.test(l)?"price-asc":/category/.test(l)?"category-asc":/status/.test(l)?"status-asc":/z-a|descending/.test(l)?"name-desc":"name-asc";manageSort.menu=k;manageTab="menu";switchView("manage");copilotMessages.push({role:"assistant",text:`Done — I opened <b>Manage → Menu</b> sorted by <b>${esc(k.replace("-asc"," ASC").replace("-desc"," DESC"))}</b>.`});return renderCopilot();}
  if(/inventory.*(sort|low)|sort.*inventory|reorder first/.test(l)){manageSort.inventory="stock-asc";manageTab="inventory";switchView("manage");copilotMessages.push({role:"assistant",text:"Done — I opened <b>Manage → Inventory</b> with the lowest-stock ingredients first."});return renderCopilot();}
...
function renderSettings() {
  ...
  <div class="card" style="padding:16px;max-width:700px"><div class="form-grid">
    ...
    <div class="field full"><label>DATA STORAGE</label><div class="form-note">Menu, categories, series, ingredients, recipes, discounts, expenses, sales and inventory movements are stored locally on this iPad using browser storage. The POS is designed to keep working without a server connection. Use Backup regularly to protect your data.</div></div>
